...
pub async fn read_config() -> Result<ConfigData, String> {
    ...
    if path.exists() {
        ...
        return Ok(ConfigData { raw, path: path.to_string_lossy().to_string() });
    }
    Ok(ConfigData { raw: "{}".into(), path: path.to_string_lossy().to_string() })
}
...
fn extract_port_from_config(raw: &str) -> Option<u16> {
    let config: serde_json::Value = serde_json::from_str(raw).ok()?;
    config.get("gateway")?.get("port")?.as_u64().map(|v| v as u16)
}
...
pub async fn detect_gateway_config() -> Result<GatewayConfigInfo, String> {
    ...
    if path.exists() {
        if let Ok(raw) = std::fs::read_to_string(&path) {
            ...
            if let Some(p) = extract_port_from_config(&raw) { port = p; }
            if token.is_some() { found_path = Some(path.to_string_lossy().to_string()); }
        }
    }
