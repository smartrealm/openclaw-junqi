...
- [x] 丰富业务侧栏的当前平台、Session、Agent、工具与最近操作摘要，不增加新的运行时状态源。
- [x] 删除筛选栏重复 Profile 输入，增加真实业务域与操作效果计数。
- [x] 增加官方审计与本窗口投影的派生摘要、来源范围和文本筛选。
- [x] 增加插件安装与 Gateway 重启的不定进度、确认失败内联反馈和重启优先判定。
- [x] 更新独立 HTML 预览，使侧栏与筛选结构和生产页面保持一致，并移除静态风险数量。
