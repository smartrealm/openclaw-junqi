...
11. 已完成：将已发送消息的 `sessions.rewind` 入口改为明确的“编辑并重新发送”，把分叉收进带文字的更多菜单，保留复制直达和独立协作入口，并补充消息操作层级与本地化回归。
...
输入区主操作、状态订阅和已发送消息操作层级已完成。消息操作现在以“编辑并重新发送”为可见主入口，复制保持直接操作，分叉进入更多菜单，协作入口独立分组；断线和运行等禁用状态提供具体原因。定向回归和 TypeScript 检查通过，完整前端测试、lint、生产构建和真实 Tauri 视觉验收结果以 `PROJECT_STATUS.md` 为准。当前源码的真实 Tauri 输入法、活动运行、暗色与窄窗口连续视觉状态尚未完成，没有用旧单实例窗口或模拟 Gateway 状态替代。
