...
2. 对每个受影响读取、读写、`sessions.patch` 组织及 `sessions.compaction` 检查点入口复用既有会话目标守卫，删除默认参数并阻止空 key 进入 mutation coordinator。
...
- `src/services/gateway/SessionCompactionClient.test.ts`
