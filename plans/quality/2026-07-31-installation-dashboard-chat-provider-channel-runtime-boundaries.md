...
10. 将执行追溯的默认摘要收敛为 Agent 名称、会话语义和来源记录计数；原生标识移入技术详情，并在主聊天与快捷聊天共用的侧栏状态中实现只读来源记录下钻。
...
- 后续人工验收应覆盖超过初始历史页范围的来源记录，确认 Gateway 仅对来自 `chat.history` 的原生消息标识执行 `chat.message.get`。
