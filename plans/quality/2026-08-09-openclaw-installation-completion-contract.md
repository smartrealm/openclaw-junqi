...
2. 安装完成门禁单元测试。
3. 首次启动与 Wizard 回归测试。
4. `pnpm lint`、`pnpm test` 和 `pnpm build`。
5. Tauri 本地安装包与真实 Gateway 流程验收。
