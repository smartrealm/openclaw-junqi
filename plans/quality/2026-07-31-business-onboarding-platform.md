...
1. 完成：修复引导已查看状态的版本迁移和重新打开语义，旧状态不会在升级后重置。
2. 完成：将总览面板限制为总览路由，并提供经当前 runtime 门禁约束的顶部全局入口与独立弹窗层。
3. 完成：提取只读渠道状态适配器，并用现有账号就绪判定投影渠道任务。

## 2026-08-02 交互导览实施顺序

1. 完成：复核会话、模型服务、智能体、渠道和工作区页面的既有操作入口，并以稳定 `data-tour` 属性作为跨路由导览契约。
2. 完成：将总览任务点击改为从相应步骤启动；覆盖层仅遮挡非目标区域，真实目标点击后推进，跳过不改变领域完成状态。
3. 完成：将恢复入口固定在 Header 的通知入口左侧，并覆盖三种语言。
4. 完成：定向测试、TypeScript 检查、生产构建和差异检查均通过；桌面窗口中的实际视觉表现另行记录。
