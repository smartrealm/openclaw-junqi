...
- [x] 依据最新版官方 Voice Wake 文档复核全局 trigger 与路由键的不同匹配语义，分离
  trim-exact 列表比较与路由归一化，防止删除或授权其他客户端的变体 trigger。
- [x] 核对 `voicewake.routing.set` 没有运行时消费者且当前规格禁止路由写入，删除无 UI
  与无授权流程的 Gateway 写包装，同时保留路由读取和事件核验。
