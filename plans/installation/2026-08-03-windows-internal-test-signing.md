...
- 临时 runner 通过 Windows `certutil -f -addstore` 在本地计算机 `TrustedPeople` 与 `TrustedPublisher` 中信任公开 CER，以执行 Authenticode 验证，不修改受保护的根证书库，并在 `always()` 精确清理当前用户私钥与本地计算机信任项。
