...
用户提供的首次配置 GIF 已证明旧安装包会在 Wizard 最终提交导致 Gateway 重载、进程内会话丢失后重跑完整官方向导。源码已加固为只接受当前官方 `wizard.next` 的结构化完成响应；会话丢失时保留“终态未知”，不自动重放，也不再调用 OpenClaw 未定义的配置检测 RPC。当前源码与本地安装包已经更新，下一阶段是在包含上游会话保留修复的 OpenClaw Runtime 上复验同一路径。
...
首次设置页面已按完整 JunQi 链路统一，而非只处理官方 Wizard：欢迎、环境检测、数据位置、运行方式、依赖安装、Gateway 启动、OpenClaw 配置和完成页共用固定步骤器、页面主内容滚动区与固定操作区。步骤卡片不再成为通用滚动容器，也不再预留固定滚动槽；日志正文和官方长选项只在确有需要时局部滚动。安装与 Wizard 继续使用各自真实状态机；页面只统一几何边界、状态反馈、键盘焦点和 140 至 180 毫秒的轻量过渡，不增加假步骤、假进度或伪完成。
...
- `detecting` 与 `environment-review` 现在共享同一视觉场景和组件实例；页面主内容区滚动通过稳定引用在绘制前
  复位，不再以 React key 重建滚动容器及其异步子树。
- 首次设置滚动所有权已收敛到 `SetupShell` 的页面主内容区。步骤卡片删除通用 `overflow-y-auto` 与固定 `scrollbar-gutter`，避免每一步都出现卡片内滚动槽；固定底部操作仍保持可达，日志正文和官方长选项继续使用有界局部滚动。
- 数据位置步骤的“安装位置”首次进入时默认展开，工作区及可选 npm、Node.js 和 Git 位置无需额外点击即可查看；设置草稿继续保存用户在同一次设置会话中的显式展开或折叠选择。
- 首次设置步骤卡片已取消通用最小高度和剩余空间强制填充，内容区按真实步骤自适应。正文已经承载官方 message 时不再显示“当前内容由所选 Runtime 提供”的客户端占位副标题。普通官方步骤提交时保留当前上游内容并禁用重复交互，不再用三层重复文案替换；授权提交和官方自动进度步骤仍按协议生命周期销毁旧二维码或旧操作，显示紧凑等待状态。
...
- 已从提交 `37fb09f1` 重新生成 macOS ARM64 安装包。镜像通过 `hdiutil verify`，SHA-256 为 `eadde19290704fd4b76c03d6500aa3f15fd2ab100509d3b122e382967a6f97f0`，大小为 8,317,905 字节；镜像内应用版本为 3.1.0，Bundle ID 为 `com.junqi.junqidesktop`，可执行文件为 ARM64。应用仅有 ad-hoc 本地签名，且本机没有 updater 私钥，因此该制品只用于本机安装验证，不是正式签名或公证发布包。
...
- 本次首次设置页面定向回归 34 项通过，覆盖稳定主体、页面主内容区滚动、内容过渡、存储加载、运行方式单飞、正文去重、通用提交等待、`Done` 终态等待、错误替换与恢复操作。
...
- 本次滚动边界回归先在旧实现上得到 4 项失败；修复后首次设置骨架、日志、存储、环境复核、官方向导和长选项相关定向测试 37 项通过。完整 `npm test` 共 238 项通过；`npm run lint` 通过，模块边界检查覆盖 893 个文件，版本一致性与 TypeScript 检查通过；`npm run build` 生产构建通过。
- 本次安装位置默认展开、内容自适应和官方步骤提交态回归先在旧实现上得到 3 项失败；修复后相关定向测试 42 项通过。完整 `npm test` 共 238 项通过；`npm run lint` 和 `npm run build` 通过。包含本次调整的 macOS ARM64 安装包已生成并通过镜像校验；亮色、暗色和窄窗口真机视觉复验仍待完成。
...
5. 使用当前 macOS ARM64 本地安装包复验首次设置的滚动边界、安装位置展开状态和官方步骤提交状态；未经明确要求不推送或发布。
