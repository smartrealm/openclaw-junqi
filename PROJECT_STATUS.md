...
全面对齐当前所选 OpenClaw Runtime 的正式首次安装能力，并完成 OpenClaw 官方配置终态到 JunQi 工作台的单一、可核验交接链路。配置模式依据正式 RPC 响应协商，不绑定版本号。当前优先收敛最新版 Guided 候选梯子、setup admission busy、检测信息呈现和取消链路。
...
- 安装日志由共享页面骨架统一控制，所有步骤默认收起；依赖安装默认显示执行时间线，只有用户主动选择“执行记录”才挂载日志正文，等待、失败和步骤切换不会自动展开。
...
- 安装步骤的日志展开状态属于共享展示层，不属于各步骤业务状态；默认值固定为收起，不能由失败或内容高度改变。
...
- 完整前端测试 2719 项通过，脚本测试 238 项通过；新增定向回归 63 项通过，覆盖候选梯子、路径确认、配置占用分类、检测详情呈现、两类取消、数据位置提交、日志控制与 Gateway 生命周期协调。
...
- 本轮数据位置提交、日志默认收起、Gateway 交接和 Guided 对齐修复已通过自动化验证；是否包含于安装包必须以本轮之后重新生成的制品为准。

## 已知问题与未验证边界

- 最新版 Guided 真实 provider 登录、浏览器授权、取消后的官方终态以及自动激活后的改选路径尚未在支持当前协议的 Runtime 上完成真机端到端验证。
- 当前数据位置显式确认、提交期间表单稳定、日志默认收起、Gateway 连接代次、短步骤内容高度和国际化资源修复尚未完成连续页面真机验收。
