...
收敛会话进度空态、钉钉接入页的 Gateway 所有权误判、DWS 多账号入口、协作办公室配置入口与钉钉登录身份、Session 策略、业务权限三层展示，并定位聊天输入卡顿链路。前述代码修改、定向验证和本地 Apple Silicon DMG 重新打包已完成；输入性能问题已完成静态根因核对，尚未实施修复或真机性能录制；当前公开 Release 仍不包含本轮修复。
...
- 最终 DMG 修改时间为 `2026-08-20 18:54:08`，大小为 `8367674` 字节；经 `hdiutil verify` 校验有效，SHA-256 为 `d54c60d78de652c27f54377b7035f4a5f593abbbb939234476900c0467a7be24`。
...
- 当前公开 `v3.2.1` Release 不包含本轮修复；最新本地 DMG 已包含进度空态、Gateway 身份再核验、DWS 多账号和钉钉三层权限语义修复，但尚未打标签或发布新版本。
...
7. 安装最新本地 DMG，在真实 Tauri WebView 验收 Profile 下拉和登录身份、Session 策略、业务权限三层语义。
