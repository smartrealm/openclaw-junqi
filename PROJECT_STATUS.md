...
- 已基于当前工作区构建并校验 macOS arm64 DMG 验收包：`src-tauri/target/release/bundle/dmg/JunQi Desktop_2.2.10_aarch64.dmg`。`hdiutil verify` 通过，文件大小为 8742356 字节，SHA-256 为 `4595d64f617d04861fc744d9d222aee2668a2229fb15b93c096152239260c041`；安装后确认 `/Applications/JunQi Desktop.app` 版本为 `2.2.10`、可执行文件为 arm64 且哈希与包内一致，用户反馈正常可用。该包未正式签名或公证，完整证据见 `docs/installation/setup-round-trip-navigation-validation-2026-08-08.md`。
...
- Setup 步骤切换动效尚未完成真实 Tauri 帧时间与低性能设备验收；当前已验证方向计算、当前页入场状态、旧退出页代码清理、环境按钮门禁和 TypeScript 编译，不把动画完成当作业务状态完成。
