...
- 已从提交 `1d9a4ec0b3d8` 和当前未提交的首次设置滚动修复重新生成 macOS ARM64 安装包。镜像通过 `hdiutil verify`，SHA-256 为 `5c3832799501541de9f7731da5f2359dcc3ae271d742efac36b6fd4f7c7f7643`，大小为 8,318,298 字节；镜像内应用版本为 3.1.0，Bundle ID 为 `com.junqi.junqidesktop`，可执行文件为 ARM64。应用仅有 ad-hoc 本地签名，且本机没有 updater 私钥，因此该制品只用于本机安装验证，不是正式签名或公证发布包。
...
- 本次安装位置默认展开、内容自适应和官方步骤提交态回归先在旧实现上得到 3 项失败；修复后相关定向测试 42 项通过。完整 `npm test` 共 238 项通过；`npm run lint` 和 `npm run build` 通过。尚未重新生成包含本次后续调整的 macOS 安装包，也未完成亮色、暗色和窄窗口真机视觉复验。
