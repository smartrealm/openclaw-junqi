...
JunQi Desktop `3.2.1` 已通过不可变 `v3.2.1` 标签完成三平台构建和 GitHub Release。当前交接目标是保留发布证据，并在目标设备上补齐安装、签名信任、OpenClaw 运行、DWS 全流程和 UI 真机验收。
...
- 版本提交 `5df92992c04e9420efe365b06c98e0c33acfde96` 已推送到远端 `main`，同提交 `CI` 工作流 32322901090 全部成功。
- 带注释标签 `v3.2.1` 已推送并精确指向版本提交；`Tagged Desktop Release` 工作流 32323466560 全部成功。
- GitHub Release `JunQi Desktop 3.2.1` 已发布并成为 Latest，共包含 11 个附件，覆盖 macOS ARM64、macOS x64、Windows x64、更新签名、内部测试证书说明和 `latest.json`。
...
- 远端同提交 `CI` 工作流 32322901090 通过。
- `Tagged Desktop Release` 工作流 32323466560 通过：发布源校验、macOS ARM64、macOS x64、Windows x64、Release 创建和汇总全部成功。
- GitHub Release `v3.2.1` 已核验为正式 Latest 发布，共 11 个附件；远端标签解引用后指向 `5df92992c04e9420efe365b06c98e0c33acfde96`。
...
1. 在 macOS ARM64、macOS x64 和 Windows x64 目标设备下载 `v3.2.1` Release 制品，核验摘要并完成安装、升级、卸载和首次启动测试。
