...
修复 OpenClaw 配置向导确认提示与日志呈现，并让 JunQi 的运行时语言控制面遵循 OpenClaw 原生本地化契约。
...
- “配置 OpenClaw”阶段的配置核验、向导连接和正式步骤均默认展开日志，首条日志到达前也保留空日志面板，用户可手动收起。
...
4. 在 JunQi 管理的 Native 与 Docker Runtime 上分别切换三种运行时语言，完成亮色、暗色、键盘焦点和窄窗口真机验收。
