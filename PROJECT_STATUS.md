...
当前目标是完成协作插件受控更新闭环：修复旧插件数据库错误封死更新、真实 schema 13 混合结构迁移和健康确认后的旧插件回滚制品残留，并通过新桌面安装包完成实际 Gateway 更新、数据库迁移、制品清理和智能体办公室恢复验收。
...
- 本轮根前端与脚本测试通过：前端 2933 项、脚本 238 项全部通过；`pnpm lint` 和 `pnpm build` 通过，构建内协作包仍为 `0.5.5`、schema 15 和固定摘要 `fa2ab4fc4b06bd78965a3f146ce635445aa51f0e49b1147511e0caab88cb38e7`。
...
- 先前工作区曾生成 macOS Apple Silicon 本地测试 DMG。应用与构建版本均为 `3.2.1`，二进制为 ARM64；`hdiutil verify` 通过，DMG SHA-256 为 `7d6acea3f70bf067a8cc1ed130d4b6358e1f2da9720b5a5395e540bcc73598b4`。该制品早于本轮 Rust 清理修改，不能代表当前源码。
...
- 现有 ARM64 本地测试 DMG 早于本次 Rust 插件事务清理修改，不能用于验收旧插件归档删除；需要重新打包后才能在目标 Gateway 核对制品目录。
...
3. 重新生成 ARM64 本地测试 DMG，在协作设置执行受控更新，核对实际 Gateway 运行时插件为 `0.5.5`、实际数据库为 schema 15、数据库迁移备份存在、插件更新事务制品已清理且智能体办公室恢复；正式分发前另行完成 Developer ID 签名、公证与 updater 签名。
