...
持续评估并扩展钉钉 DWS 集成，按 P0、P1、P2 顺序完成契约准入、写入核验、实时事件、日常工作流和高敏业务域。当前代码阶段已完成 P0 底座、主要读写与目标 Gateway 验收入口，并完成钉钉桌面工作台的信息层级、目录检查器布局、窄窗口聚焦、活动证据渐进披露和接入页视觉一致性重构。本轮同时收敛首次设置中“复用现有 OpenClaw”与“独立 npm 安装”的语义，并统一桌面应用图标与当前公司品牌。当前最紧急门禁是在 Docker 可用的受控测试机跑通两层隔离 Gateway 验收，以正式 DWS 和受控钉钉租户执行目标读写矩阵，同时在真实 Tauri 窗口完成宽窄布局、状态序列和目标平台视觉验收。
...
- 存储设置模型 3 项定向测试通过，新增用例确认 OpenClaw 官方 Gateway 核验失败会与普通存储错误分开呈现；三语言 JSON 解析、完整 TypeScript、模块边界和版本一致性检查通过。
...
- 本轮 `corepack pnpm test` 全仓测试、`corepack pnpm lint`、`corepack pnpm build` 和 `git diff --check` 均通过。生产构建生成的协作插件归档摘要噪声已从本任务变更中排除，没有混入图标与设置文案提交范围。
- 本机 `corepack pnpm tauri dev` 已完成编译并启动当前调试应用，进程保留给用户本地检查。启动诊断明确报告当前选定 OpenClaw 需要兼容 Node.js、但所选运行时没有发现兼容版本；这只证明当前机器的设置阻塞，不代表其他目标机器。
...
- `git diff --check` 通过。
- 对当前 9 个修改或未跟踪文本文件执行完整 Emoji 扫描，未发现命中。
...
4. 先使用已内嵌同 DWS、同 Profile 核心只读门禁的直连确认入口执行日程创建、同 ID 更新和同 ID 取消；通过后用 `dingtalk:gateway:target-calendar` 在同一目标核验工具投影、报告式审批边界、三个独立所有人审批、同 ID 读回和禁止未知写重放。随后先执行待办直连验收，再用 `dingtalk:gateway:target-todo` 对同一自分配待办完成四个报告式审批边界、五个独立所有人审批、同 ID 读回和禁止未知写重放；执行前接受目标租户保留一条完成状态的合成记录。
