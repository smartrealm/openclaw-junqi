...
办公室工作区始终以 `configuredAgents` 展示静态虚拟工位平面，分为协调席位和协作席位。工位只呈现 Gateway 已返回的名称、描述、协调者与允许参与配置，并明确标注为配置投影；它不是 Run 成员、在线、空闲或执行状态的投影。选定真实 Run 后，权威 `AgentOfficeView` 在同一工作区覆盖实际分派、尝试和人工核验状态。智能体目录网格仍保留为用户主动切换的备用视图。
...
- 复用 `AgentOfficeView`、`LoadingIndicator`、协作状态图标和状态文案；无运行时使用配置工位平面，避免把配置数据传入权威 Run 投影。智能体中心默认不以网格目录作为 Office 主视图。
