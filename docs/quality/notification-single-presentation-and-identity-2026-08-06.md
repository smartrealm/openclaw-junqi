...
流结束事件使用 `runId`，持久转录事件使用未经归一的助手 `idempotencyKey`。两者在旧 Gateway 载荷中可能为同一运行时生成不同通知键；正常路径按精确键去重，Rust 持久层另保留同会话、同角色、同标题正文、两分钟窗口的窄兼容判断，仅用于这类已知运输镜像。
...
3. 通知仓库优先按精确 `dedupeKey` 去重；仅对旧 Gateway 的同会话聊天运输镜像保留窄范围兼容判断，不作用于无键事件、跨会话事件或超出时间窗口的独立消息。

## 验证

- 定向 TypeScript 测试：73 项通过，覆盖用户、Gateway 助手与 CLI 助手的运行身份归一、媒体附属消息不归一，以及转换后持久转录仍与流式运行对应。
- 完整前端测试：通过。
- Rust 通知定向测试：17 项通过；Rust library 全量测试：713 项通过，2 项既有忽略。
