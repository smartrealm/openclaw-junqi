...
目标交互应把已持久化用户消息的主入口明确命名为“编辑并重新发送”，使用编辑图标，但底层仍只调用官方 `sessions.rewind`，并在确认文案中说明后续当前分支会被移除。分叉属于低频会话结构操作，应放入带文字的更多菜单；复制可保留为直接操作；多 Agent 协作必须与会话结构操作保持独立表达。该目标尚未实现，不能把当前截图视为已验收。
