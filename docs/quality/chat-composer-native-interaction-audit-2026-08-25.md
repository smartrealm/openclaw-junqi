...
- 最终定向测试 31 项通过；完整测试、lint、生产构建和差异检查的最新结果记录在 `PROJECT_STATUS.md`。
- 独立 Tauri 调试窗口被已运行的单实例应用接管，因此没有把旧窗口截图计为当前源码的视觉证据。
...
首次修正把多 Agent 协作改成带可见名称的独立按钮，并把后台消息身份确认显示为禁用加载按钮。真实会话截图证明该方案把内部资格探针暴露成了常驻业务状态，挤压时间与复制操作行，也让普通用户消息出现与当前意图无关的“正在确认消息身份”。

最终修正保留会话分叉的 `GitFork`，多 Agent 协作使用独立的 `UsersRound` 图标并复用共享 `ChatIconButton`。只有待启动或已有运行可查看时才显示紧凑入口，完整名称由本地化 tooltip、`title` 和 `aria-label` 提供；后台确认态返回空内容，不再生成按钮、加载标签、分隔线或布局占位。该改动只调整入口表达，不改变协作插件或 OpenClaw 协议。

组件回归覆盖待启动、查看运行和后台确认三种状态，并断言协作入口不复用会话分叉图标、后台确认态不渲染内容。真实 Tauri 的亮色、暗色、窄窗口、悬停和键盘焦点视觉仍需确认。

## 输入法高频更新修正

草稿原本写入 ChatStore，但应用根、会话页签、命令面板、状态栏、仪表盘等多个消费者使用无 selector 的 `useChatStore()`。每个组合输入更新都会通知这些无关消费者并扩大重渲染范围，输入框自身还会收到一次草稿订阅更新。这是中文输入法卡顿的直接前端状态链路，不涉及 Gateway 或消息发送协议。

修正后，应用根与输入框的非文本依赖使用可测试的最小投影并配合 `useShallow`；其余 ChatStore 消费者按单字段 selector 或浅比较对象订阅。输入框只订阅消息总数，不再订阅完整消息数组；上下方向键需要历史时才从 Store 读取当前用户消息，因此助手流式内容增长不会持续重绘 Composer。草稿仍保留原有按会话存储、发送快照和会话切换语义。

回归测试分别证明草稿变化和助手流式内容变化不会改变输入框非文本投影。真实 Tauri 中文输入法连续录屏仍未完成，自动化只证明订阅边界，不把性能推论描述为真机结果。

## 已发送消息操作再审查

2026-08-26 将 OpenClaw 官方仓库更新到主线 `e3a3de0fe7f8e91c0345a88d3e910ad6d7dc32cd` 后重新核对。官方 macOS 与 Android 客户端都把 `sessions.rewind` 表述为“回到此处”，并在成功后用响应中的 `editorText` 和 `editorAttachments` 整体替换输入框；`sessions.fork` 则创建新会话。官方移动端把两项操作放在带可见文字的消息菜单中，没有把高风险会话操作仅表达为一排无文字图标。

JunQi 当前协议调用正确：第一个 `RotateCcw` 调用 `sessions.rewind`，第二个 `GitFork` 调用 `sessions.fork`，随后依次是复制和多 Agent 协作。已持久化消息不能被本地直接改写；铅笔编辑器只用于尚未进入 OpenClaw transcript 的发送失败消息，这一数据边界正确。

交互表达不正确：普通用户会把 `RotateCcw` 理解为重试或撤销，无法知道它会截断当前分支后续路径并把原消息恢复到输入框；“重绕到此消息”也不是自然的编辑意图。`GitFork` 与紧邻的 `UsersRound` 在小尺寸下仍容易混淆，四个同权重常驻图标让复制、破坏性回退、创建分叉和启动协作混在同一层级。

目标交互应把已持久化用户消息的主入口明确命名为“编辑并重新发送”，使用编辑图标，但底层仍只调用官方 `sessions.rewind`，并在确认文案中说明活动分支会回退、既有记录仍保留且工具副作用不会撤销。分叉属于低频会话结构操作，应放入带文字的更多菜单；复制可保留为直接操作；多 Agent 协作必须与会话结构操作保持独立表达。

2026-08-27 将 OpenClaw 官方仓库继续更新到主线 `8e4f3711c20c718a0268f875fd62b9e1bf58f1a0` 后复核 `docs/web/control-ui.md`、`src/gateway/methods/core-descriptors.ts` 和 `src/gateway/server-methods/sessions-rewind.ts`。官方当前契约确认：`sessions.rewind` 将活动会话指向目标消息之前并返回编辑器内容，需要 `operator.admin`；`sessions.fork` 从活动前缀创建新会话，需要 `operator.write`；两者都只作用于已持久化消息，运行期间应禁用，且 rewind 不撤销文件或工具副作用。

修复后，用户消息直接展示带铅笔图标和可见文字的“编辑并重新发送”，复制保持无副作用的直接图标操作，分叉收进“更多消息操作”的带文字菜单项。多 Agent 协作仍由独立能力门禁拥有，并通过间距和主题分隔线与消息结构操作分组。断线、活动运行、历史加载和另一项结构变更期间，编辑与更多入口禁用，辅助名称和标题提供具体原因。底层仍只调用既有 Gateway 客户端，没有新增本地 transcript 改写、乐观成功或协议 fallback。

纯状态回归覆盖主操作、直接操作和更多菜单的分层，并验证三种语言不再暴露“重绕”入口术语。真实 Tauri 的亮色、暗色、窄窗口、悬停、键盘焦点、菜单定位以及从确认到输入框恢复的连续视觉仍待验证。
