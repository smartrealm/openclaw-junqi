...
- 在实际 `/settings?tab=appearance` 页面提供界面、等宽和编辑器字体；终端页不重复设置字体。
...
- 字体配置只有一个用户入口：`/settings?tab=appearance` 的“字体”卡片。
- 该卡片集中管理界面、等宽和编辑器字体；编辑器为空时继续继承等宽字体。
- 终端页只保留字号、光标、回滚、输入与启动器等终端行为。终端页的“恢复默认”不再清空字体选择，避免跨页面修改外观偏好。
- 字体状态、持久化键、启动前恢复及终端和编辑器渲染链路保持不变。
