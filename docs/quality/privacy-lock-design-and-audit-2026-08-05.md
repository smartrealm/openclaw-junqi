...
自动离席监控使用平台可信的系统空闲时间。由于成功解锁不会重置操作系统的最后输入时间，监控器维护 Native 侧的一次性活动闩锁：若解锁时系统已经超过阈值，必须先观察到空闲时间回落到阈值以下，随后新的超时周期才会触发 `idle` 锁定。该逻辑不依赖 Renderer 输入事件，也不伪造系统输入。

当前仍未接入 Windows/macOS 的独立系统会话锁定通知；`RunEvent::Resumed` 只覆盖 Tauri 应用恢复事件，不能等同于完整系统会话锁定信号。
