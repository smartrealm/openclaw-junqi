...
Gateway `doctor.memory.status` 是诊断协议，不是通用记忆 CRUD 接口；主动探测会请求 embedding provider。本次不把它伪装成文件浏览或写入能力。历史 `doctor.memory.remHarness` 已被官方主线删除。
...
- Gateway 诊断已单独接入 `doctor.memory.status`，实现依据、探测成本、失败状态和未验证边界见 [`Gateway 原生能力与扩展一致性审计`](gateway-native-extension-consistency-audit-2026-08-10.md)。本页的工作区文件读取边界不因此扩大。
