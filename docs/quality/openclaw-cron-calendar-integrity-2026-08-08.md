...
日历事件首次创建 Cron 提醒或在连接恢复时补发提醒，均为同一事件提交稳定的官方
`declarationKey`。OpenClaw 的 `cron.add` 已定义 declaration-key 收敛回包，因而 Gateway 已接受写入、
但客户端未收到响应时，下一次补发仍由 Gateway 收敛为同一个任务，而不是在 JunQi 侧猜测成功、自动
复制一个任务或将未知结果标记为已完成。

CronMonitor 的模板和手动创建也使用相同规则：一次点击生成一个声明键，在 `cron.add` 成功且后续读取
确认前始终复用；只有完整确认后才释放该键供下一次独立创建意图使用。这样断线、超时或读取失败后的重试
由 Gateway 返回同一任务，而不是产生重复的定时任务。

## 验证结果

- `OpenClawCronRunClient` 定向测试：管理员与读取通道隔离、方法未找到映射、终态身份核对，共 8 项通过。
- 日历调度构建器测试：跨午夜周提醒、`every` 锚点、边界规则拒绝，共 3 项通过。
- 日历 Cron 关联回归测试：删除未确认、替换顺序、创建失败、全天事件，以及同一事件重试使用稳定
  `declarationKey`，共 5 项通过。
- 日历投影测试：2 项通过。
...
- 定向测试共 20 项通过；TypeScript 类型检查、`pnpm lint`、三份应用语言 JSON 解析、全量 `pnpm test` 与
  `pnpm build` 通过。
- 2026-08-09 重跑 Cron、日历和展示定向回归共 44 项通过；其中覆盖 Gateway 声明键收敛、创建/更新/删除、
  手动运行、运行历史、调度状态、日历转换与 Agent 选择。该次命令输出仅有 Node 弃用警告，无失败。
- 2026-08-09 对手动和模板创建补充声明键创建、未知结果重试与 Cron 管理回归，共 27 项通过，并通过
  `pnpm lint` 与 `git diff --check`。输出只有 Node 弃用警告。
...
- 尚未在真实 Gateway 上人为中断 `cron.add` 响应后验证 declaration-key 的实际收敛回包；当前结论来自
  官方 schema 与 Gateway 源码，且客户端回归已固定请求形状。
