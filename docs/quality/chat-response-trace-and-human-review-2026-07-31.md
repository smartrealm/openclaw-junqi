...
- 未取得 Gateway 审批 scope 或解析确认时，不把活动中心的本地操作描述为已批准。
...
## 2026-08-01 工具来源记录的非破坏性结果投影

### 依据

- Gateway transcript 是面向 UI 的标准化历史记录，但工具结果可经实时事件、历史 `result` / `output` / `toolOutput` 字段或内容块数组进入 Desktop。
- OpenClaw 文档未为第三方工具正文承诺统一的嵌套数据结构。因此 JunQi 只解析完整、受限的 JSON 文档和已识别的内容块，不为单个工具或当前运行环境构造专用字段规则。

### 当前行为与目标行为

- 原行为：工具结果在标准化后只保留字符串展示值。来源记录对多层序列化内容只能尝试一次 JSON 解析，失败后把转义 JSON、内容块包装和外部内容说明整体呈现。
- 目标行为：标准化层额外保留工具结果值。来源记录优先投影为内容块、对象或数组的可读结构；内容块按数据字段形状处理，不按具体工具或块类型名称建立展示分支。未知块保留为结构化对象；当前已加载的原始载荷始终可折叠查看。
- 对 Gateway 已标识的外部不可信内容包装，默认结果保留外层元数据并展示分隔符后的正文；正文为完整 JSON 时继续按结构渲染。安全包装和原始正文不会被删除，仍在原始载荷中可核对。
- 解析有 512000 字符输入、6 层嵌套和 2000 节点上限。实时和历史结果继续复用既有的工具输出长度上限，截断后不保留超出展示边界的对象。无法证实为完整结构化文档的内容保持原文；不可信外部内容保留明确的数据展示标识；截断事实继续可见。

### 验证边界

- 自动化覆盖历史结构化值保留、内容块解包、双层转义 JSON、无效 JSON 的失败关闭和不可信外部内容标识。
- 尚未用真实 Gateway 的该条历史记录完成桌面真机验收；也未验证超过展示上限的大型外部工具结果或历史分页后的原始载荷一致性。
