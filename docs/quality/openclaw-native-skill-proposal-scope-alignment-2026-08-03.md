...
本次没有接入 `skills.proposals.inspect`、history、events、evaluate 或任何管理员写方法。虽然
scope 已可见且受控，proposal detail 的完整 record、revision hash、support files、内容渲染与
生命周期操作还需要独立的 schema、内容安全和交互审查，不能从清单状态推断。
