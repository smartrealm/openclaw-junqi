...
- `OpenClawApprovalClient` 保留 OpenClaw 原生的两个 pending list 方法，同时接入统一
  `approval.history`、`approval.get` 和 `approval.resolve`。`hello-ok.features.methods` 仅作保守发现，
  不作为这些 RPC 的发送门禁。统一响应只接受官方脱敏 `ApprovalSnapshot`，涵盖 exec、plugin 和 system-agent，并保留 Gateway 的
...
- `hello-ok.features.methods` 的遗漏不阻止某个 list 方法的真实请求。Gateway 明确返回
  method-not-found 时，该方法标记为不可用；认证、传输、参数或响应错误继续向 UI 抛出，
  不静默降级。
...
- `OpenClawApprovalClient.test.ts` 覆盖 exec/plugin list、统一 history/resolve、system-agent
  presentation、方法发现遗漏仍调用、method-not-found、严格响应、允许决策校验、
