...
侧栏会话行和标签页此前各自以鼠标坐标创建固定定位容器。虽然容器通过 portal
挂载到 `document.body`，但没有统一的视口碰撞处理，边缘位置会使操作菜单的图标或文字
落在可视区域外。

现改为共享 `FloatingMenuPortal`：菜单继续在文档级挂载，并统一处理点击外部、Escape、
窗口尺寸变化、菜单内容尺寸变化和边缘避让。三点菜单以右边缘锚定，不再依赖固定菜单宽度。

此修正不改变 OpenClaw 会话 RPC、组织字段或操作权限。已补充视口边缘与右对齐定位的纯函数
回归测试；仍需在 macOS、Windows、Ubuntu 和 CentOS 桌面制品上人工验证实际右键交互。

## 菜单交互对齐（2026-08-05）

再次核对本地官方 OpenClaw 源码提交 `1e3880352e6` 的
`ui/src/components/session-menu.ts` 与 `ui/src/components/sidebar-menus-render.ts`。此前 JunQi 虽共享了
菜单容器，但菜单内仍以“重命名优先、分组内联展开”的方式组织，并混入关闭标签页与重置会话，
与官方会话组织菜单的交互不一致。

现有菜单按官方顺序展示组织操作：置顶、已读状态切换、重命名、分叉、移动分组、归档和删除。
分组在同一文档级菜单宿主内向侧面展开，靠近右侧边缘时自动翻转到左侧，并依据实际测量高度避让
视口。新建分组从子菜单进入应用内对话框，确认后先由 Gateway 确认分组，再写入会话分组字段。
会话行继续承担导航，标签关闭仍由标签本身承担，重置仍只在错误恢复入口提供。

`setSessionUnread(key, unread)` 现在直接投影 Gateway 已确认的 true/false 值；此前仅能写未读，
无法实现官方“标记为已读”的反向操作。该旧单向 store 方法已移除。新增的纯定位测试和状态仓测试
覆盖侧向翻转、底部避让以及已读状态双向投影。真实 Gateway 的分组权限与各桌面平台的指针交互仍需
在目标环境验收。
