...
- `BUG-DWS-13`，高优先级：DWS 操作活动时，弹窗的关闭请求被代码直接忽略；取消按钮没有进入取消中状态，且取消 RPC 额外依赖会话运行时身份。用户既无法确认取消是否已请求，也可能因身份变化无法终止 JunQi 自己创建的授权 CLI 子进程。
...
- DWS 授权弹窗的关闭、Escape 和遮罩关闭请求在启动、运行或取消中统一视为取消意图；启动响应尚未返回时保留该意图，取得精确 `operationId` 后立即取消。取消中显示明确状态并拒绝重复点击，终态仍以子进程退出事件为准。
- 取消 RPC 只接收 JunQi 当前持有的 `operationId`，后端只终止与该标识匹配的本地 DWS CLI 子进程。取消不读取或改写 DWS 凭据，不代表浏览器页、外部授权或业务副作用已经撤销；只有用户从关闭动作发起且收到取消终态时才收起对话框。
...
- 2026-08-27 DWS 取消定向前端回归通过，覆盖活动三态和关闭即请求取消；Tauri IPC 回归确认取消只传递精确 `operationId`。Rust DWS 定向测试、格式化和库编译通过。Rust 官方 `Child::kill` 与 `Child::try_wait` 文档表明终止请求后仍须等待退出状态收集；实现保留该终态事件，未把取消请求描述为外部授权已回滚。
...
- DWS 授权取消尚未在真实 Native、Docker、Windows 或 Linux 运行时连续验收；当前只证明 JunQi 的弹窗、IPC 和本地子进程生命周期，不证明浏览器自动关闭、DWS 服务器端授权撤销或钉钉业务副作用回滚。
