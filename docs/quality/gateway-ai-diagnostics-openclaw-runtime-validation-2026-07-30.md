...
## 2026-08-01 Gateway 自救面板状态收敛

### 问题与原因

状态栏曾将健康状态下的重启说明作为 `progressMessage` 和 `error` 传给自救面板。面板据此显示进度、恢复建议、官方修复和 AI 诊断，造成“已连接”与“需要修复”同时出现。面板还默认拼接本机回环地址，不能代表所有运行环境。

### 当前行为

- 已连接且空闲：显示 Gateway 控制、实际 endpoint 或端口、重启和查看日志。
- 处理中：显示完整进度和进度条，不把该信息交给错误诊断。
- 未连接或收到实际失败：显示恢复建议、官方修复和 AI 诊断。
- endpoint 仅从调用者提供的 Gateway URL 解析；不可用时只显示实际端口，不假定主机地址。

### 验证边界

- 自动化覆盖健康、处理中和恢复状态的动作可见性。
- 仍需在 Native、Docker 和远程 Gateway 的桌面真机中确认 endpoint、重启进度和日志跳转。
