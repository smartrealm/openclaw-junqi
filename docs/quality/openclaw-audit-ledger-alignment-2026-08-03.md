...
JunQi 的响应追溯现在优先调用 Gateway 的 OpenClaw 原生审计方法，并由正式 RPC 响应决定兼容路径：

- 先调用版本化、metadata-only 的 `audit.activity.list`。
- Gateway 对该方法返回正式未知方法错误时，才对运行和工具过滤回退到 `audit.list`。
- 两个官方 RPC 均实际不可用，或查询需要消息方向、消息渠道而只有旧方法时，显示上游不支持，不伪造数据。
- `hello-ok.features.methods` 仅可辅助发现，遗漏不阻止请求；这不是版本判断，也不是对能力的乐观推断。
...
- `src/services/gateway/Connection.ts` 只保留连接、身份围栏、超时和协议帧边界，不以 `features.methods` 作为本地调用许可表。
...
- Gateway 重连后不复用旧连接的请求或结果；每次查询都以当前连接的正式响应为准。
...
- `OpenClawAuditClient` 测试覆盖新协议优先、未知方法后的旧协议回退、消息过滤限制、终态 error code 关联和 metadata-only 过滤
- `Connection` 测试覆盖发现遗漏时仍发送、连接围栏和断开清理
