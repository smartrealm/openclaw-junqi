...
JunQi uses `OpenClawSessionBranchesClient` for branch listing and switching, and `OpenClawSessionMessageCutClient` for message-level rewind and fork. The connection preserves the authenticated Gateway handshake's advertised method list for the lifetime of its socket. New controls are not rendered when the Gateway omits or does not advertise their methods. This avoids treating an older installed Gateway as compatible merely because a newer upstream source contains the method.
...
- Authorization, transport, and response-validation errors remain intact and are not converted to local fallback behavior.
...
- The unified session toolbar lists branches only when `sessions.branches.list` is advertised, and enables switching only when `sessions.branches.switch` is also advertised.
...
- `node --import ./test-setup.ts --import tsx --test src/services/gateway/sessionCapabilities.test.ts src/services/gateway/OpenClawSessionBranchesClient.test.ts src/services/gateway/OpenClawSessionMessageCutClient.test.ts`
- `git diff --check`
