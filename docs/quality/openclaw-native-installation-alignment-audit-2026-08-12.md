...
官方短步骤保持轻量表面，以左侧细引导线、语义图标底和眉题色区分提示、待执行与处理中状态。正文使用 `aegis-text-secondary` 提高可读层级，Runtime 来源说明使用 `aegis-text-muted`，来源图标沿用当前状态语义色。其余颜色全部复用 `aegis-surface`、`aegis-border`、`aegis-primary-surface`、`aegis-warning-surface` 及对应文字 token，不新增亮色或暗色专属值，也不把整张卡片涂成高饱和状态色。
