...
### BUG-SS-07 · 严重 · 创建顺序在稀疏时间字段下失效

首轮实现直接以会话行可选的 `createdAt` 排序。当前 Gateway 行缺少该字段时全部得到相同排序值，只能退回当前数组顺序；会话列表因最近活动刷新而变化后，“创建时间”无法保持稳定。OpenClaw 官方侧栏维护独立的稳定创建顺序，并在客户端确认新会话后把该会话提升到首位。

影响：

- 旧会话或稀疏响应切换到“创建时间”后没有可见变化。
- 新会话和后续轮询可能改变已有会话的相对创建顺序。
...
- 创建排序复用官方稳定顺序模型：首次看到的 Gateway 行记录其相对顺序，已确认的新会话提升到首位；最近更新排序仍读取 Gateway 时间字段。该顺序只影响侧栏呈现，不写回 OpenClaw。
