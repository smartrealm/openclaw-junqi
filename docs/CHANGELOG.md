...
本次版本提升桌面萌宠在复杂壁纸下的文字可读性，并完成一轮全局代码规范化。

**桌面萌宠可读性与隐私**

- 萌宠状态气泡会根据附近桌面区域的派生亮度和对比度，自动调整文字前景、描边、阴影和轻量背景；深色和浅色壁纸均保持可读。
- 采样不再以短周期持续执行：仅在萌宠移动、状态变化、窗口恢复可见时防抖刷新，并保留 90 秒低频兜底。
- macOS 改用 CoreGraphics 内存采样，Windows 使用 Desktop DC 内存位图；原始桌面像素不写入文件、不经 IPC 传给前端、不上传。
- 采样区域由窗口与显示器几何关系动态选择，萌宠贴近边缘时自动切换另一侧，避免读取萌宠自身的透明表面。
- 设置页和通用设置弹窗新增“自动调整文字对比度”开关，关闭后立即清空读数并停止原生采样；中英阿三种语言均已覆盖。

**运行稳定性与代码结构**

- Agent 任务启动参数收敛为 `TaskLaunchRequest`，前端 IPC 与 Rust 使用同一 camelCase 请求契约，并新增反序列化回归测试。
- OpenClaw npm 安装参数收敛为 `NpmInstallRequest`，降低安装链路中参数顺序错配的风险。
- 登录 shell PATH 在 Tauri 启动前初始化；应用设置加载时会规范化历史快捷键值。
- 清理无行为 Gateway 接口和未消费状态，收紧 Windows 专用 Git/Node 安装代码的条件编译边界；OAuth 浏览器打开改用现有系统默认浏览器能力。

**平台边界**

- 当前稳定发布通道以 Windows 10+ 为支持范围。Windows 7 的 WebView2/Edge 支持已终止，单独替换萌宠窗口不能让整个 WebView2 应用恢复为受支持配置。
- 详细平台审查与剩余原生能力验收项见 `docs/pet-platform-surface-audit.md`。

**质量检查**

- Rust：361 项通过。
- 前端：815 项通过；脚本：27 项通过。
- `cargo clippy --lib --tests -- -D warnings`、Rust 格式、TypeScript、模块边界检查、生产构建和差异检查均通过。
