...
Wizard 会话存在于 Gateway 进程内。Gateway 重启后，原 `sessionId` 可能得到 `WIZARD_NOT_FOUND`。JunQi 只能清除该运行时范围内的旧标识并重新启动官方 Wizard；不得重放旧答案或根据本地缓存伪造完成状态。
...
- 开始新的完整流程前，若当前范围仍有会话，客户端先请求官方取消；会话已经丢失时清除旧标识。
