...
`setup-session.log` 只有一个 5 MB 备份；同一次超长安装第二次轮换时会删除最早部分，无法保证一次安装从开始到失败的连续证据。
...
`reset_timeline_log` 使用覆盖写，镜像或安装器重试后只能看到最后一次尝试。
...
目录创建、文件打开和写入错误均被忽略；磁盘满或权限变化时用户仍会误以为已有完整日志。
...
winget 只在进程结束后保留最多 1200 字符；npm 重复警告和 Gateway 长行会因界面策略丢失。
...
日志能看到等待提示，却不能稳定关联进程启动、PID、退出码和总耗时。
...
用户只能打开目录，远程排查时需要手动寻找并选择多个文件。

**修复**：提供只读快照 ZIP 导出，跳过符号链接和导出目标自身，并在安装控制台提供本地化入口。
