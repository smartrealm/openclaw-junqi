...
Gateway 就绪状态、继续说明和运行时完成度；顶部第三阶段以高亮勾选表示“本阶段已完成，尚未进入下一阶段”。安装时间线与实时
日志默认折叠在“查看安装详情”中，避免固定高度详情制造无意义的外层滚动。点击下一步后的配置核验仍停留在该检查点，只有
确认需要官方 Wizard 时才进入第四阶段，已有配置满足门禁时可直接进入完成阶段。
