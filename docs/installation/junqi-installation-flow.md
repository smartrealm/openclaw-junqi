...
2. JunQi 按所选运行时检测或准备 Node、npm、OpenClaw、Docker 与必要系统能力。路径和凭据始终绑定该运行时，不能使用开发机默认值。Native Node.js 只有同时满足当前目标 OpenClaw 包的 `engines.node` 范围并提供同一发行版自带 npm 时才会复用；已检测但不兼容的版本必须显示实际版本和目标范围。系统 PATH 同时存在版本管理器与系统安装器提供的多个 Node.js 时，JunQi 会按 PATH 顺序核验全部可执行候选并选择首个满足范围且自带 npm 的完整运行时，不能因排在首位的旧版本不兼容而忽略后续候选。安装入口在下载或系统安装前再次核对这一完整契约，发现兼容运行时后直接复用。
3. 数据位置步骤会读取选定运行时绑定的真实目录，但初次读取只保持最终表单的静态骨架，不把短暂 IPC 阶段展示为独立页面。初始读取不递归统计旧 OpenClaw 目录容量；迁移事务内的复制完整性统计继续保留。读取完成后始终停留在可编辑表单，不能自动进入下一阶段；用户点击“下一步”且原生存储事务成功后才进入运行时。选择当前目录且未修改原生运行时位置或配置路径时，该事务只持久化布局，不要求尚未安装的 OpenClaw 核验官方 Gateway 服务；只有真实数据迁移、原生运行时位置变化、配置路径变化或未完成的服务恢复会进入服务身份核验。待恢复事务明确记录没有所选官方服务时，布局回滚不会额外要求兼容 Node.js；若先前服务或运行中的 Gateway 需要恢复，仍严格核验对应启动契约和健康状态。如果数据位置提交发现已登记的 Gateway 服务，但 Node.js 不满足 OpenClaw 要求，页面把主操作切换为“修复 Node.js 并继续”；只有用户点击后才调用可取消的现有 Node 安装或 npm 修复契约，成功后自动重试原存储事务。修复失败保留原服务门禁和本次诊断，不修改数据位置。恢复失败页会显示可执行提示并默认收起技术详情，用户可以返回环境检测修复依赖；返回只保留待恢复事务，重新进入数据位置页后仍必须恢复成功才能修改位置。端口占用时页面先显示脱敏进程名、PID 和归属提示；只有端口、PID、启动代次与当前用户在确认执行时仍一致，才提供“关闭进程并恢复”。该操作先请求正常退出，必要时才强制结束，并在端口释放后自动继续恢复；占用者变化或权限不匹配时不会终止。正常表单默认展开“安装位置”，直接展示 OpenClaw 工作区及可选的 npm、Node.js 和 Git 位置；用户可以折叠该区域，在同一次设置会话中返回此步骤时保留用户的展开或折叠选择。
...
5. 认证连接建立后调用最新版正式 `openclaw.setup.detect` 协商配置协议。成功时继续使用同一组正式 Guided RPC；Gateway 明确返回 unknown-method 时进入同一 Runtime 的官方 Classic Wizard。无权限、断线与响应非法分别保留真实失败，不能冒充协议不支持，也不能通过旧方法名重试。
   本次设置开始前已安装的 Native Runtime 在“正在配置 JunQi Desktop”完成后显示“下一步”，进入独立的“OpenClaw 更新”步骤并自动执行一次当前官方渠道更新检查；检查完成前不能启动配置 RPC，检查失败时原地重试，可用更新仍由用户明确确认，用户也可以不更新并通过“跳过更新并继续”直接进入配置。一次无错误的结构化检查已确认渠道可管理后，后续更新执行失败不会抹除继续配置资格；错误仍在更新区域原地显示，用户可以重试更新，也可以继续使用现有版本。真正的检查失败、未知渠道、beta 和 dev 仍阻断。更新进度事件只负责展示，完成与放行必须来自同一次 Tauri 检查请求的结构化返回；开发模式的 Effect 监听器清理与重建不得作废仍在执行的请求，真实卸载或后续请求换代仍会阻止旧结果发布。执行更新前先要求当前所选 Runtime 存在已核验 Gateway 连接，缺失时只经统一生命周期入口重连；协作服务正常时仍必须取得维护租约。维护门禁从能力接口只投影实例标识、Schema 版本和数据库完整性三个稳定字段，再读取正式维护状态，不因旧插件缺少与维护无关的新版展示字段而提前失败。协作能力接口精确证明服务启动失败或数据库 Schema 不受支持时，恢复宿主 OpenClaw 的更新可以暂时无租约继续。旧 Gateway 没有注册该接口时，还必须由绑定同一已核验连接的本地探针证明协作插件已安装但未加载、无维护事务和警告、允许桌面修改且持久状态可读。两类证明都要在实际写入前重读；服务恢复、故障变化、插件已加载、活动任务、身份漂移、状态损坏和未知错误均停止更新。本次流程中新安装的 OpenClaw 不重复进入该步骤，运行时配置完成页显示“核验配置”并直接准备官方配置。Gateway 认证与运行时身份核验成功时，Node.js、npm、OpenClaw 和 Gateway 的展示状态会一次性收敛为完成或已跳过，不保留恢复路径遗留的等待与进行中状态。随后协商配置协议并准备首个可操作状态：Guided 先形成候选确认或选择状态，Classic 先取得 `wizard.start` 或 `wizard.resume` 返回的首个官方步骤；准备完成后才切换到配置页。Classic 首次使用主线正式参数 `installDaemon:false`；只有 stable 以 `INVALID_REQUEST` 精确拒绝该字段且尚未创建会话时，才以 `mode` 与规范化 `workspace` 这组共同正式参数启动一次。其他参数、权限、网络和业务错误不重放。准备失败会在更新页就地呈现真实错误和重新核验操作，不先进入空配置页，也不依赖配置页挂载后自动启动向导。
...
8. Guided 与 Classic 的官方终态共用唯一交接门禁：终态只接受 `done === true`，随后重新读取所选 Runtime 的端点与凭据，在同一认证连接上要求 `config.get.configRevisionHash` 与 `appliedConfigHash` 非空且相等。配置尚未应用时等待 OpenClaw 自身重载和认证连接轮换；普通超时不主动重启。只有 `gateway.reload.mode` 明确为 `off`，或官方 `health.configReload.hotReloadStatus` 明确为 `disabled` 时，才经统一 Gateway 生命周期入口补发一次重启。修订生效后再核验所选 Runtime；Guided 必须执行 detect 与 verify，不能以客户端推断或 activate 的旧结果代替最终核验，也不能为核验自动重放有写入副作用的 activate。Classic 采用当前官方 Wizard 的终态。进入 Ready 前必须还是同一条已核验连接和同一个活动配置修订；修订变化会在原事务预算内重新完成证据链，任一步失败都停在当前配置页，不重启向导、不恢复旧二维码、不写入完成标记。
   已有 Classic Runtime 且本次不写配置时，不属于上述 Wizard 写后交接。用户可以跳过可选更新；JunQi 在同一已核验连接上读取有效的 `config.get` 与稳定 `hash`，通过唯一临时 Session 调用官方 `agent` 和 `agent.wait` 做真实模型核验，随后再次核对连接与 `hash` 并删除临时 Session。只有完整成功才直接进入 Ready，不要求重复运行官方向导。该证据不能替代未来任何配置写入后的活动修订核验。
   JunQi 已在运行时阶段安装并启动所选 Gateway，因此主线完整配置提交 `installDaemon:false`，由统一 Gateway 生命周期继续拥有安装和启动。stable 明确不接受该字段时，后续 daemon 选择由官方 Wizard 步骤完整呈现，JunQi 不改写答案或伪报已关闭。真正的会话丢失继续保留终态未知和人工确认；参数协商只发生在官方确认未创建会话的 schema 拒绝之后。
...
- React 开发模式会额外执行 Effect 的 setup 与 cleanup 周期：[`StrictMode`](https://react.dev/reference/react/StrictMode)
