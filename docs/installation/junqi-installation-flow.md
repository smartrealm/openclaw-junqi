...
3. 数据位置步骤默认展开“安装位置”，直接展示 OpenClaw 工作区及可选的 npm、Node.js 和 Git 位置。用户可以折叠该区域；在同一次设置会话中返回此步骤时保留用户的展开或折叠选择。
4. JunQi 启动或复用 Gateway，并在认证连接与 Runtime Identity 均完成核验后继续。端口可达或进程启动不等于交接成功。
5. 首次配置默认进入同一 Gateway 的官方 Wizard。JunQi 不从配置文件、端口健康或未注册的检测方法推断可以跳过；既有配置和可跳过步骤由官方 Wizard 自己决定。
6. Wizard 的模型、凭据、工作区、渠道及可跳过步骤均按其结构化步骤呈现。确认步骤的提示只在其确认控件中显示一次；正文已经承载官方 message 时不再补充客户端占位副标题。配置核验与向导连接阶段默认展开日志，正常交互步骤默认收起，步骤失败时自动展开，用户始终可以手动切换。首次设置页面由主内容区统一承担纵向滚动，步骤卡片按内容自适应且不预留固定滚动槽；步骤切换必须复位主内容区滚动位置，不能让上一状态的滚动偏移把下一状态移出视口。普通官方步骤提交时保留并禁用当前正文，只通过主操作显示处理状态；授权提交和官方自动进度步骤才替换为紧凑等待状态。
...
8. 官方 Wizard 返回 `done` 后，JunQi 只执行一次系统服务交接，并等待属于该交接的新 WebSocket `hello-ok` 和 Runtime Identity。最终 `wizard.next` 尚未返回时若旧 `sessionId` 被回收，JunQi 重新连接所选 Gateway 并尝试恢复原会话；确认会话不存在后保留“终态未知”，不自动启动新的 Wizard，也不从配置、健康或旧步骤推断完成。用户重新开始时必须经过可取消的二次确认，并明确获知可能重复配置写入的风险。官方终态后的交接失败只允许“重新核验”，不重复启动 Wizard。全部核验成功后才进入 Dashboard。
