...
Gateway 就绪是第三阶段“运行时”的完成检查点，不新增顶部步骤，也不与第四阶段 OpenClaw 配置合并。页面保留第三阶段标识、
Gateway 就绪状态、下一步配置核验说明和运行时检查数；顶部第三阶段以高亮勾选表示“本阶段已完成，尚未进入下一阶段”。检查点
只显示一次 `已完成数/总数`，不再重复“就绪”和 100% 进度。安装时间线与实时日志默认折叠在“查看安装详情”中，避免固定高度
详情制造无意义的外层滚动。点击下一步后的配置核验仍停留在该检查点；检查中或核验失败只改变页面当前动作，不会把已完成的
运行时检查点改写成进行中或失败。只有确认需要官方 Wizard 时才进入第四阶段，已有配置满足门禁时可直接进入完成阶段。
