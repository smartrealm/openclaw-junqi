...
- 在修改代码前先完成必要的事实核对并形成简洁计划，明确根因、影响链路、实施顺序和验收方式；简单任务可在工作说明中直接列明，复杂任务必须维护可更新的分步计划。
- 修复缺陷时优先依靠源码、测试、日志和正式契约自主推进。只有缺少的用户决策会实质改变结果、扩大授权范围或引入不可逆风险时才提问；不得用连续追问代替调查和判断。
- 完成实现后必须按风险执行定向测试、静态检查、构建或真实运行验证，并清楚区分自动化通过、当前机器实测和目标平台实测。未验证的内容不得描述为正常或完成。
- 命令失败、外部接口异常、用户纠正或发现更可靠方法时，先查明原因，再按自我改进流程记录可复用结论；适用于全仓的经验要提炼进 `AGENTS.md` 等长期规范，避免重复犯错。
- 禁止临时拼凑、表面修补和掩盖错误。修复必须落在真实根因和正确抽象边界上，并同步删除被替代的旧路径、无效 fallback、重复实现及其专属测试和文档。
- 以资深开发者的标准对结果负责：主动核对上下游契约、异常路径、并发与生命周期、权限、安全、跨平台、国际化和可维护性，持续推进到可验证的闭环；证据不足时如实标记边界，不猜测成功。

### 长时异步任务等待

- 长时异步任务不需要中间输出时，空 `write_stdin` 轮询的 `yield_time_ms` 必须不小于 `180000`，优先使用 `300000`。
- `functions.wait` 的 `yield_time_ms` 必须不小于 `180000`。
- `functions.exec` 的外层 `@exec yield_time_ms` 必须比内部工具最长等待时间至少多 `30000` 毫秒，避免外层代码单元先行让出。
- 向交互式进程发送非空输入的 `write_stdin` 不适用上述长等待要求。
- 等待工具会在进程或代码单元完成时提前返回；不得仅为报告任务仍在运行而主动唤醒模型。
- 提交和发布使用组合 shell 命令时必须启用立即失败与管道错误传播；任一前置扫描或校验失败后不得继续暂存、提交、创建 tag 或推送。
...
- 查阅 OpenClaw 源码时，必须先更新官方 GitHub 仓库，并以最新远端主线或任务明确指定的 tag、commit 为准。不得以全局 npm、项目 `node_modules` 或其他安装目录中的源码替代官方仓库；这些安装内容只用于本地复现和兼容性证据。
...
- 每个持续任务结束、交接、暂停或重要阶段完成前，必须回写根目录 `PROJECT_STATUS.md`。内容仅保留当前目标、
  已完成内容、关键技术决策、核心文件、测试与验证、已知问题、失败方案和下一步顺序；必须依据当前代码、
  Git 变更和实际运行结果核对，不得记录密码、密钥、token、个人信息或未经验证的推测。
...
- Gateway 启动、停止、重启、恢复、健康重试与故障收敛必须只经过一个统一生命周期协调入口；页面、Hook、设置项和诊断页不得直连进程或 Gateway command，也不得维护第二套重试、轮询、状态机或成功语义。所有入口复用同一动作状态、错误映射、禁用规则和运行时身份围栏，重启相关 UI 必须使用同一共享展示组件与文案来源。
...
- 不保留向后兼容、历史迁移、旧接口包装或兼容 fallback。需求替换旧行为时，必须在同一变更中删除过时实现、迁移代码、测试、配置与文档，不能保留双轨路径。
...
- 异步加载、轮询或多探针聚合页面不能只验收最终静态截图。必须连续抓帧或录屏核对 loading 到结果的完整序列，并检查结果是否原子发布、组件是否重挂载、React key 是否重建主体、动效是否造成位移以及晚到数据是否触发二次布局变化。
- 后台资格确认、能力探针和身份核验不得以常驻按钮、加载标签或占位控件挤压消息正文和主操作栏；仅在动作真实可用时显示紧凑入口，等待态只在用户主动发起且需要反馈的操作附近呈现。
- 输入法组合、文本输入、拖拽和流式增量等高频状态不得通过无 selector 的 Zustand 全量订阅向应用其他区域扇出。订阅必须选择最小字段集合，并以投影稳定性或连续交互的渲染证据验证无关区域不会随高频更新重绘。
...
- 无论任务类型，均不得参考、读取或引用代理所在本地环境的 Node.js、npm、pnpm、PATH、已安装依赖、运行进程、凭据、配置、网络、操作系统、用户目录、命令输出或工具版本来形成事实结论、选择实现、编写文档或报告验证结果。事实依据只能来自用户提供材料、仓库受版本控制的源码与文档、目标系统的可复现证据，以及相应上游的官方文档、官方源码或正式协议；缺少这些依据时必须明确标为待验证。
