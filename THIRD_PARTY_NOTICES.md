# Third-Party Notices

## Kooky terminal visual assets

The terminal workbench includes the Onest and JetBrains Mono font files and
agent icon artwork from the local Kooky reference project.

- Kooky: MIT License, copyright 2026 kooky contributors.
- Onest: SIL Open Font License 1.1.
- JetBrains Mono: SIL Open Font License 1.1.
- Agent icons: Kooky lists their upstream sources and licenses in its
  `NOTICE.md`; corresponding notices remain applicable.

## Voice wake dependencies

- sherpa-onnx 1.13.4: Apache License 2.0.
- tauri-plugin-autostart 2.5.1: Apache License 2.0 or MIT License.
