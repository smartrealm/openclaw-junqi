import type React from "react";

export const task = {
  taskPanel: {
    width: 260,
    flexShrink: 0,
    background: "var(--aegis-surface)",
    borderRight: "1px solid var(--aegis-border)",
    display: "flex",
    flexDirection: "column",
    overflow: "hidden",
  },
  taskPanelCollapsed: {
    width: 52,
    alignItems: "center",
    gap: 12,
    padding: "10px 0",
    boxSizing: "border-box" as const,
  },
  taskPanelCollapsedBody: {
    display: "flex",
    flexDirection: "column" as const,
    alignItems: "center",
    gap: 10,
    flex: 1,
    width: "100%",
  },
  taskPanelCollapsedFooter: {
    display: "flex",
    flexDirection: "column" as const,
    alignItems: "center",
    gap: 6,
    width: "100%",
    paddingTop: 8,
    borderTop: "1px solid var(--aegis-border)",
  },
  taskPanelExpandBtn: {
    position: "relative" as const,
    width: 32,
    height: 32,
    border: "1px solid var(--aegis-border)",
    borderRadius: 8,
    background: "var(--aegis-card)",
    color: "rgb(var(--aegis-text-muted))",
    cursor: "pointer",
    display: "inline-flex",
    alignItems: "center",
    justifyContent: "center",
  },
  taskPanelCollapsedNewBtn: {
    width: 32,
    height: 32,
    border: "1px solid var(--aegis-border)",
    borderRadius: 8,
    background: "var(--aegis-card)",
    cursor: "pointer",
    display: "inline-flex",
    alignItems: "center",
    justifyContent: "center",
  },
  taskPanelCollapsedSmallBtn: {
    width: 28,
    height: 28,
    border: "none",
    borderRadius: 6,
    background: "transparent",
    color: "rgb(var(--aegis-text-dim))",
    cursor: "pointer",
    display: "inline-flex",
    alignItems: "center",
    justifyContent: "center",
  },
  taskPanelAttentionDot: {
    position: "absolute" as const,
    top: 3,
    right: 3,
    width: 8,
    height: 8,
    borderRadius: "50%",
    background: "var(--danger)",
    border: "2px solid var(--aegis-surface)",
    boxSizing: "border-box" as const,
  },
  panelHeader: {
    height: 48,
    display: "flex",
    alignItems: "center",
    gap: 8,
    padding: "0 10px 0 6px",
    borderBottom: "1px solid var(--aegis-border)",
    flexShrink: 0,
  },
  backBtn: {
    background: "none",
    border: "none",
    cursor: "pointer",
    padding: "4px 6px",
    borderRadius: 5,
    display: "flex",
    alignItems: "center",
    color: "rgb(var(--aegis-text-muted))",
    flexShrink: 0,
  },
  panelCollapseBtn: {
    background: "none",
    border: "none",
    cursor: "pointer",
    padding: 4,
    borderRadius: 5,
    display: "flex",
    alignItems: "center",
    color: "rgb(var(--aegis-text-dim))",
    flexShrink: 0,
  },
  panelProjectName: {
    fontSize: 13,
    fontWeight: 600,
    color: "rgb(var(--aegis-text))",
    overflow: "hidden",
    textOverflow: "ellipsis",
    whiteSpace: "nowrap",
    flex: 1,
  },
  panelSearchWrap: {
    display: "flex",
    alignItems: "center",
    gap: 7,
    margin: "8px 10px 4px",
    padding: "6px 9px",
    background: "var(--aegis-card)",
    border: "1px solid var(--aegis-border)",
    borderRadius: 6,
  },
  panelSearchInput: {
    flex: 1,
    border: "none",
    outline: "none",
    background: "transparent",
    color: "rgb(var(--aegis-text))",
    fontSize: 12.5,
  },
  newTaskRow: {
    display: "flex",
    alignItems: "center",
    gap: 8,
    width: "calc(100% - 20px)",
    margin: "6px 10px 2px",
    padding: "6px 10px",
    border: "none",
    cursor: "pointer",
    textAlign: "left",
    borderRadius: 6,
    transition: "background 0.1s",
  },
  taskActionsRow: {
    display: "flex",
    alignItems: "center",
    justifyContent: "space-between",
    gap: 8,
    padding: "6px 19px 4px",
  },
  taskActionsMeta: { fontSize: 11, color: "rgb(var(--aegis-text-dim))", letterSpacing: 0.2 },
  taskActionBtn: {
    display: "inline-flex",
    alignItems: "center",
    gap: 5,
    padding: "4px 8px",
    border: "1px solid var(--aegis-border)",
    borderRadius: 6,
    background: "var(--aegis-card)",
    color: "rgb(var(--aegis-text-muted))",
    fontSize: 11.5,
    cursor: "pointer",
  },
  taskDivider: { height: 1, background: "var(--aegis-border)", margin: "4px 10px 2px" },
  branchBar: {
    display: "flex",
    alignItems: "center",
    gap: 6,
    margin: "0 10px 4px",
    padding: "5px 9px",
    background: "var(--aegis-card)",
    border: "1px solid var(--aegis-border)",
    borderRadius: 6,
    cursor: "pointer",
    userSelect: "none" as const,
    transition: "background 0.1s",
  },
  branchBarName: {
    flex: 1,
    fontSize: 12,
    fontWeight: 500,
    color: "rgb(var(--aegis-text-secondary))",
    overflow: "hidden",
    textOverflow: "ellipsis",
    whiteSpace: "nowrap" as const,
  },
  branchDialogBox: {
    width: 360,
    background: "var(--aegis-card)",
    border: "1px solid var(--aegis-border)",
    borderRadius: 12,
    boxShadow: "var(--shadow-popover)",
    padding: "20px 20px 16px",
    display: "flex",
    flexDirection: "column" as const,
    gap: 14,
  },
  branchDialogTitle: {
    fontSize: 14,
    fontWeight: 700,
    color: "rgb(var(--aegis-text))",
    marginBottom: 2,
  },
  branchInput: {
    width: "100%",
    padding: "8px 10px",
    background: "var(--aegis-input)",
    border: "1px solid var(--aegis-border)",
    borderRadius: 8,
    color: "rgb(var(--aegis-text))",
    fontSize: 13,
    fontFamily: "var(--font-ui)",
    outline: "none",
    boxSizing: "border-box" as const,
  },
  branchOptGroup: {
    fontSize: 10.5,
    fontWeight: 700,
    color: "rgb(var(--aegis-text-dim))",
    letterSpacing: 0.7,
    padding: "6px 10px 3px",
    textTransform: "uppercase" as const,
    userSelect: "none" as const,
  },
  branchDialogError: { fontSize: 12, color: "var(--color-error)", padding: "4px 0" },
  taskListScroll: {
    flex: 1,
    overflowY: "auto",
    padding: "2px 0 12px",
    overflowAnchor: "none" as const,
  },
  taskPanelFooter: {
    padding: "10px 10px",
    borderTop: "1px solid var(--aegis-border)",
    display: "flex",
    justifyContent: "flex-start",
    background: "var(--aegis-surface)",
  },
  taskListEmpty: {
    padding: "24px 16px",
    fontSize: 12,
    color: "rgb(var(--aegis-text-dim))",
    textAlign: "center",
  },
  groupLabel: {
    fontSize: 10.5,
    fontWeight: 700,
    color: "rgb(var(--aegis-text-dim))",
    letterSpacing: 0.7,
    padding: "10px 14px 3px",
    textTransform: "uppercase" as const,
  },
  taskCard: {
    display: "flex",
    alignItems: "flex-start",
    gap: 9,
    padding: "7px 12px",
    margin: "1px 6px",
    borderRadius: 6,
    cursor: "pointer",
    transition: "background 0.1s",
  },
  taskCardTitle: {
    fontSize: 12.5,
    fontWeight: 500,
    color: "rgb(var(--aegis-text))",
    overflow: "hidden",
    textOverflow: "ellipsis",
    whiteSpace: "nowrap",
  },
  taskCardSub: { fontSize: 11, color: "rgb(var(--aegis-text-muted))", marginTop: 1 },
  taskDiffStats: {
    display: "inline-flex",
    alignItems: "center",
    gap: 6,
    marginLeft: 8,
    fontSize: 11,
    fontVariantNumeric: "tabular-nums",
  },
  taskDiffAdditions: { color: "var(--diff-add-fg)", fontWeight: 600 },
  taskDiffDeletions: { color: "var(--diff-delete-fg)", fontWeight: 600 },
  agentBadge: {
    flexShrink: 0,
    width: 14,
    height: 14,
    display: "inline-flex",
    alignItems: "center",
    justifyContent: "center",
  },
  worktreeBadge: {
    position: "absolute" as const,
    right: 36,
    top: 12,
    flexShrink: 0,
    width: 12,
    height: 12,
    display: "inline-flex",
    alignItems: "center",
    justifyContent: "center",
    color: "rgb(var(--aegis-text-muted))",
    pointerEvents: "none" as const,
    transition: "opacity 0.12s ease",
    zIndex: 1,
  },
  taskDeleteBtn: {
    flexShrink: 0,
    width: 22,
    height: 22,
    display: "inline-flex",
    alignItems: "center",
    justifyContent: "center",
    border: "none",
    borderRadius: 5,
    background: "transparent",
    color: "rgb(var(--aegis-text-dim))",
    cursor: "pointer",
    transition: "opacity 0.12s ease, background 0.12s ease, color 0.12s ease",
  },
  taskPlayBtn: {
    flexShrink: 0,
    width: 22,
    height: 22,
    display: "inline-flex",
    alignItems: "center",
    justifyContent: "center",
    border: "none",
    borderRadius: 5,
    background: "transparent",
    color: "rgb(var(--aegis-primary))",
    cursor: "pointer",
    transition: "opacity 0.12s ease",
  },
  taskStarBtn: {
    flexShrink: 0,
    width: 22,
    height: 22,
    display: "inline-flex",
    alignItems: "center",
    justifyContent: "center",
    border: "none",
    borderRadius: 5,
    background: "transparent",
    cursor: "pointer",
    transition: "opacity 0.12s ease, color 0.12s ease",
  },
  taskRenameBtn: {
    flexShrink: 0,
    width: 22,
    height: 22,
    display: "inline-flex",
    alignItems: "center",
    justifyContent: "center",
    border: "none",
    borderRadius: 5,
    background: "transparent",
    color: "rgb(var(--aegis-text-dim))",
    cursor: "pointer",
    transition: "opacity 0.12s ease, color 0.12s ease",
  },
  taskRenameInput: {
    flex: 1,
    minWidth: 0,
    fontSize: 12.5,
    fontWeight: 500,
    color: "rgb(var(--aegis-text))",
    background: "var(--aegis-input)",
    border: "1px solid rgb(var(--aegis-primary))",
    borderRadius: 4,
    padding: "1px 5px",
    outline: "none",
  },
} satisfies Record<string, React.CSSProperties>;
