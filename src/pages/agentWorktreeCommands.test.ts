...
test('agent worktree IPC payloads match the Rust command field names', () => {
  assert.deepEqual(createTaskWorktreeArgs('/repo', 'task-1', ''), { projectPath: '/repo', taskId: 'task-1', baseBranch: 'main' });
  ...
  assert.deepEqual(worktreeDiffStatsArgs('/repo', '/worktree'), { projectPath: '/repo', worktreePath: '/worktree', baseBranch: 'main' });
});
