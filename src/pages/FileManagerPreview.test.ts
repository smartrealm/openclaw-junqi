...
test('FILE-01 binary file-manager previews use a scoped URL rather than the text reader', () => {
  ...
  assert.match(fileManager, /hasFileKind\(selected\.ext, 'pdf'\) && binaryPreview/);
  assert.match(fileManager, /workspaceFileKind/);
  assert.doesNotMatch(fileManager, /const PDF_EXTS/);
});
...
test('FILE-02 HTML file-manager previews use the scoped protocol or a scriptless static fallback', () => {
  ...
  assert.match(fileManager, /hasFileKind\(selected\.ext, 'html'\) && selected\.exists && htmlPreview\?\.kind === 'html'/);
  ...
  assert.match(fileManager, /if \(hasFileKind\(selected\.ext, 'html'\)\) return;/);
});
