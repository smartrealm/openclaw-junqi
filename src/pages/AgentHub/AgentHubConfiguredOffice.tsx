...
export function AgentHubConfiguredOffice({
  ...
  return (
    <section className="w-full text-left" aria-labelledby="agent-hub-configured-office-title">
      ...
      <div className="grid gap-3 lg:grid-cols-[minmax(220px,0.9fr)_minmax(0,2fr)]">
        <section className="overflow-hidden rounded-lg border border-aegis-primary/20 bg-aegis-primary/[0.035]" aria-labelledby="agent-hub-coordination-zone">
        ...
        <section className="overflow-hidden rounded-lg border border-aegis-border bg-aegis-surface" aria-labelledby="agent-hub-work-zone">
          <div className="px-3 py-3">
          <header className="flex items-center gap-1.5 text-[10.5px] font-semibold text-aegis-text-secondary">
          ...
          <div className="divide-y divide-aegis-border border-t border-aegis-border">
            ...
            {allowed.length === 0 && unavailable.length === 0 && <span className="block px-3 py-3 text-[10px] text-aegis-text-dim">{t('agentHub.office.configuredNoWorkers', '暂无配置席位')}</span>}
          </div>
        </section>
      </div>
    </section>
