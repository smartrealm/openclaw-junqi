...
test('terminal tabs physically stop their exact run before UI removal', () => {
  ...
  assert.ok(close.indexOf('await stopWorkbenchPty') < close.indexOf('closeStoreTab'));
...
test('group close uses atomic ownership validation and preserves UI on failure', () => {
  ...
  assert.ok(close.indexOf('await stopWorkbenchPtys') < close.indexOf('removeStoreGroup'));
