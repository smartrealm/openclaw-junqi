...
async function loadDeps() {
  ...
  const { ChatHandler } = await import('@/runtime/OpenClawChatEventRuntime');
...
test('chat.delta projects official deltaText when no message snapshot is present', async () => {
  installWindowMock();
  const { ChatHandler } = await loadDeps();
  resetChatStore();
  ...
  assert.equal(chunks.at(-1), '你好');
});

test('chat.status projects the official startup phase until visible activity begins', async () => {
  installWindowMock();
  ...
  assert.deepEqual(useChatStore.getState().chatRunStartupBySession[sessionKey], {
    runId,
    phase: 'preparing_context',
  });
...
test('durable session.message only exposes the session and role for unread projection', async () => {
  installWindowMock();
  const { ChatHandler } = await loadDeps();
  resetChatStore();

  const notices: Array<{ sessionKey: string; role: string }> = [];
  const handler = new ChatHandler({
    callbacks: {
      onStreamChunk: () => {},
      onStreamEnd: () => {},
      onTranscriptMessage: (notice: { sessionKey: string; role: string }) => notices.push(notice),
    },
  } as any);
