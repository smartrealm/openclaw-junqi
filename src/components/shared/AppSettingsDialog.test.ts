...
test('application settings expose the JunQi skill hub panel', () => {
  ...
  assert.match(source, /new Event\('junqi:skill-hub-changed'\)/);
});
...
test('about panel follows the JunQi product-card structure without internal feature claims', () => {
  assert.match(source, /<JunQiLogo variant="emblem"/);
  assert.match(source, /github\.com\/smartrealm\/openclaw-junqi/);
  assert.doesNotMatch(source, /JunQi 39-feature port/);
