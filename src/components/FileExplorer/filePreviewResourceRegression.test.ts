...
test("BUG-PREVIEW-MISSING-04 a file that leaves the disk pauses saving instead of closing its tab", () => {
  const viewer = read("./FileViewer.tsx");
  ...
  assert.match(viewer, /\{diskReadError \? \([\s\S]{0,120}<FileUnavailableBanner onRetry=\{\(\) => void reloadFromDisk\(\)\}/);
  assert.match(viewer, /disabled=\{\s*Boolean\(diskReadError\)/);
