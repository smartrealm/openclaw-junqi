...
const previewPaneSource = readFileSync(new URL("./FilePreviewPane.tsx", import.meta.url), "utf8");
const documentHookSource = readFileSync(new URL("./useWorkspaceFileDocument.ts", import.meta.url), "utf8");
...
test("file viewer defaults markdown to preview and supports explicit durable save", () => {
  ...
  assert.match(previewPaneSource, /onOpenLocalLink=\{openLocalMarkdownLink\}/);
  assert.match(previewPaneSource, /resolveMarkdownResourcePath\(href, filePath, projectPath\)/);
  assert.doesNotMatch(previewPaneSource, /dangerouslySetInnerHTML/);
  assert.match(previewPaneSource, /event\.key\.toLowerCase\(\) !== "s"/);
  assert.match(previewPaneSource, /saveNow/);
  ...
  assert.match(tabBarSource, /createPortal\([\s\S]*menu[\s\S]*document\.body/);
});
