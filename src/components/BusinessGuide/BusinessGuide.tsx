...
import { Check, Circle, Compass, Play, X } from 'lucide-react';
...
export function BusinessGuide() {
  ...
  const hasSession = useChatStore((s) => s.sessions.some((session) => (
  ...
  useEffect(() => {
    if (!tourOpen || !step.selector) { setTarget(null); return; }
    let active = true;
    const find = () => {
  ...
  }, [step.selector, tourOpen, location.pathname]);
  ...
  return <>
    {dismissed ? <button type="button" title={t('businessGuide.reopen')} aria-label={t('businessGuide.reopen')} onClick={() => useBusinessGuideStore.getState().reopen()} className="absolute right-5 top-5 z-10 flex h-8 w-8 items-center justify-center border border-aegis-border bg-aegis-surface text-aegis-text-muted shadow-sm hover:text-aegis-text"><Compass size={16} /></button> : showOverview && <section className="mx-5 mt-5 border border-aegis-border bg-aegis-surface shadow-sm" aria-label={t('businessGuide.title')}><div className="flex items-start justify-between gap-4 border-b border-aegis-border px-5 py-4"><div><h2 className="text-sm font-semibold text-aegis-text">{t('businessGuide.title')}</h2><p className="mt-1 text-xs text-aegis-text-muted">{t('businessGuide.progress', { completed, total: tasks.length })}</p></div><button type="button" onClick={dismiss} aria-label={t('businessGuide.dismiss')} className="p-1 text-aegis-text-muted hover:text-aegis-text"><X size={16} /></button></div><ul className="grid divide-y divide-aegis-border md:grid-cols-2 md:divide-x md:divide-y-0">{tasks.map((task) => <li key={task.id} className="flex items-center gap-3 px-5 py-3">{task.state === 'completed' ? <Check size={16} className="text-emerald-500" /> : task.state === 'blocked' ? <Circle size={16} className="text-aegis-text-muted" /> : <Play size={16} className="text-aegis-primary" />}<button type="button" onClick={() => navigate(task.route)} className="min-w-0 text-left"><span className="block text-xs font-semibold text-aegis-text">{t(task.titleKey)}</span><span className="block text-[11px] text-aegis-text-muted">{task.state === 'blocked' ? t('businessGuide.blocked') : t(task.descriptionKey)}</span></button></li>)}</ul></section>}
    {tourOpen && <div className="pointer-events-none fixed inset-0 z-[2147481000]" role="dialog" aria-modal="true" aria-label={t('businessGuide.title')}>
      ...
      <div className="pointer-events-auto fixed w-[min(420px,calc(100vw-32px))] border border-aegis-border bg-aegis-surface shadow-2xl" style={target ? { left: Math.max(16, Math.min(window.innerWidth - 436, target.left)), top: Math.min(window.innerHeight - 250, target.bottom + 18) } : { left: '50%', top: '50%', transform: 'translate(-50%, -50%)' }}><div className="flex justify-end px-4 pt-3"><button type="button" onClick={closeTour} aria-label={t('businessGuide.dismiss')} className="text-aegis-text-muted hover:text-aegis-text"><X size={18} /></button></div><div className="px-6 pb-6"><p className="text-xs font-semibold text-aegis-primary">{t('businessGuide.step', { current: index + 1, total: steps.length })}</p><h2 className="mt-2 text-lg font-semibold text-aegis-text">{step.title}</h2><p className="mt-3 text-sm leading-6 text-aegis-text-muted">{step.description}</p></div><div className="flex items-center justify-between border-t border-aegis-border bg-aegis-bg px-4 py-3"><button type="button" onClick={closeTour} className="text-xs text-aegis-text-muted">{t('businessGuide.skip')}</button><div className="flex gap-2"><button type="button" disabled={index === 0} onClick={() => setIndex((current) => Math.max(current - 1, 0))} className="border border-aegis-border px-3 py-2 text-xs disabled:opacity-40">{t('common.back')}</button>{step.selector ? <button type="button" onClick={() => setIndex((current) => Math.min(current + 1, steps.length - 1))} className="border border-aegis-border px-3 py-2 text-xs text-aegis-text-muted">{t('businessGuide.skipStep')}</button> : <button type="button" onClick={() => index === steps.length - 1 ? closeTour() : setIndex((current) => current + 1)} className="bg-aegis-primary px-3 py-2 text-xs font-semibold text-white">{index === steps.length - 1 ? t('businessGuide.finish') : t('common.next')}</button>}</div></div></div>
    </div>}
  </>;
}
