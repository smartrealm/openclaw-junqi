...
历史问题：浏览器连续听写和 native VAD 回调直接操作 composer 或发送附件，模式、turn 和目标身份没有独立 authority。2026-08-04 起浏览器连续听写已移除，桌面采集只保留原生路径。
...
历史问题：Web Speech 把文本填入 composer，native VAD 把 WAV 直接发送。2026-08-04 起 Web Speech 已移除，原生 VAD 只创建音频确认草稿。

目标：原生 VAD 先交给 coordinator，用户在主窗口确认后才发送普通音频附件。协作入口继续只接受持久用户消息。
...
| VWS-02 | 原生 VAD 先生成音频草稿；音频只在确认后走普通附件发送。 | `useComposerVoice.test.ts` 与 coordinator 测试。 | 真实媒体与 Gateway 音频理解。 |
...
- 本次不把 VAD 或静态测试称为真实 Wake、真实 Gateway 或正式发布验收。
