...
2. `sessions.fork` sends `{ sessionKey, entryId, agentId? }` and creates no optimistic local session.
...
5. Method-not-found is distinct from authorization, pairing, transport, and response-validation failures.
