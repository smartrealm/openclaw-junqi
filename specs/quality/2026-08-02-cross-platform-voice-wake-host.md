...
- Jarvis model and wake-phrase configuration is available at `/settings?tab=jarvis`; the conversation composer retains only the current-session start/stop shortcut. Opening settings does not claim the microphone, create a Talk session, or alter Gateway routing.
...
- The Dynamic Island close control hides its own auxiliary window immediately, then asks the main window to persist the disabled preference. A lost cross-window event cannot leave the close control without visible effect.
