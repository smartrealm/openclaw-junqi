...
3. 远端 Stop 只能调用既有 `gateway.abortChat`；该 facade 继续保留会话 identity、精确 Run
   确认和 history reconciliation。
...
6. `gateway.abortChat` 必须接收显式、非空会话键；不得将缺失目标回落到 `agent:main:main`
   或其他 JunQi 生成的默认键。校验发生在 Gateway 请求之前。
...
- 空或仅空白的 Stop 目标不发送 `sessions.abort`；显式目标继续保持 Run 查询和 Gateway
  请求的同一会话身份。
...
- 不改动 OpenClaw 协议、Tauri IPC、Rust 宿主或发送队列所有权。
