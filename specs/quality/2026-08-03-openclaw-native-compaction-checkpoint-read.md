...
1. 只使用官方 `sessions.compaction.list` 和 `sessions.compaction.get`，且只传 schema 允许的 `key`、可选 `agentId` 与 `checkpointId`。
2. 请求必须经过已认证 connection fence，并在方法未广告、连接变化或断线时失败关闭。
...
- 合法 Gateway 响应能显示 checkpoint 列表和选中详情。
