...
- `FileViewer` 只负责编排；读取/监听/保存、格式能力、标签菜单和具体渲染保持独立可测试边界。
- 磁盘不可读期间不得执行已排队、即时或关闭前保存；恢复读取后才能继续写入。
...
- [x] 定向和全量自动化验证通过。
- [x] 工作区预览统一到 typed `read_file_preview` IPC 与共享格式解析器。
- [x] `FileViewer` 及拆分后的核心模块均控制在 226 行以内。
- [x] 源码编辑器在静态 CSS 中维护横向 scroller、固定 gutter 和正文结构契约。
- [x] 行号只作为 gutter 与正文并排显示，不会成为文件内容或堆叠在正文上方。
- [ ] 完成 Tauri 桌面交互验收。
