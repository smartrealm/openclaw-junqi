...
- Agent 会话状态提示保留该会话的 profile；无会话覆盖时显示“继承：Gateway 解析标签”，不得改写为客户端的“自动”。
- 同次暂存同时切换模型和思考等级时，在任何持久化前拒绝该组合；新模型 profile 必须先经 `sessions.changed` 触发的权威列表刷新确认。
- Gateway 回执确认后才更新目标会话的 `thinkingLevel`，并严格拒绝非法回执。
- 所有复用会话设置客户端的 `sessions.patch` 写入都必须携带当前明确选择的非空 session key；没有目标时
  不得开启 mutation lane、发送 RPC 或回退到固定主会话。
...
4. 三种支持语言包含继承、带默认值的继承和能力集缺失提示；紧凑触发器、弹层和 Agent 状态提示均展示 Gateway label。
5. 空活动会话时，模型、思考及其他会话设置覆盖不会发送 `sessions.patch`，且显示本地化错误。
