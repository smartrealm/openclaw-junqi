...
- 技能页面不得展示当前协议未提供 command 支撑的删除、目录导入、SkillHub CLI 或
  ClawHub CLI 操作；当前版本的 ZIP 归档上传必须遵循独立的 `skills.upload.*` 规格。
...
- 不为当前协议不存在的删除或本地归档安装功能发明 RPC；官方 `skills.upload.*` 上传除外。
