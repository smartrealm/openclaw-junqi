...
4. Jarvis category 只能由已确认的 Gateway wake trigger 派生；写入 category 前必须
   确保该 category 已存在于 Gateway catalog。
...
- 一个 wake word 触发后，其 category 已在官方 catalog 且当前 session 的官方 category
  匹配；任一步失败不接受唤醒。
