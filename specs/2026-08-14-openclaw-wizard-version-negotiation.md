...
依据：OpenClaw 官方主线提交 `57b14335665b6d8d9b66dbe7e7234dce08a91c7e` 的 `plugins inspect` 实现确认，单插件结构化报告只携带归属于目标插件的诊断；全局发现诊断仍可写入标准错误。因此 JunQi 不得把 CLI 警告文本本身当作宿主不兼容证明，必须独立核验已安装插件清单中的正式兼容范围。
...
3. `junqi.collab.capabilities` 精确返回 `SERVICE_START_FAILED` 或 `DATABASE_SCHEMA_UNSUPPORTED` 时，OpenClaw 恢复更新可在无租约状态继续。旧 Gateway 没有注册该 RPC 时，还必须由绑定同一连接的本地探针精确证明插件已安装但未加载、无维护事务、允许桌面修改且持久状态为可读的存在或缺失。普通待修复状态必须没有警告；只有探针先核验安装目录、插件包名、插件标识、插件版本、目录安全性和无运行时依赖，再从该插件清单的正式 `openclaw.compat.pluginApi` 范围证明当前 Gateway 版本不兼容时，才允许保留旧 CLI 的兼容性警告。插件已加载、完全缺失但持久状态不明、探针忙碌、清单或版本范围不可解析、状态损坏均不满足该证明。
...
- 精确服务启动失败、数据库 Schema 不支持，或旧 Gateway 上同一受管插件被权威探针证明已安装但未加载时，OpenClaw 更新可继续且不伪造维护租约；其中宿主版本不兼容必须同时由已核验插件清单的 `pluginApi` 范围和当前 Gateway 版本证明。
- 服务状态在写入前变化时更新操作不执行。
...
## WIZ-COMPAT-08 · 更新维护探针复用已核验 Node.js

当前：原生 OpenClaw 安装可能由 npm 启动器指向 JavaScript 入口。更新器已经选出兼容 Node.js，但协作维护与恢复探针只固定 JavaScript 入口并直接执行，进程解释器重新由通用 PATH 决定。系统同时存在旧 Node.js 时，同一安装会被更新器判定可用、被维护探针判定不可用，最终阻断更新。

目标：

1. 原生协作变更目标必须同时固定已核验的 OpenClaw 入口、Node.js 可执行文件、状态目录和配置文件。
2. 插件探测、安装、恢复与更新前证明必须复用同一 `NativeOpenclawRuntime` 启动契约，不得直接执行解析后的 JavaScript 文件。
3. Docker 目标继续使用容器启动器；未携带原生运行时证明的测试目标和非原生目标不得伪造 Node.js 绑定。
4. 固定目标仍必须保留输出上限、超时、状态目录、配置文件和禁止自重启的现有围栏。

验收：回归测试使用一个会输出错误结果的 JavaScript 启动器和一个独立的已核验 Node.js 启动器，要求固定原生目标只产生已核验启动器的输出；真实更新前探针与官方更新命令必须报告同一 OpenClaw 版本。
