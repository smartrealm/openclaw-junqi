...
- 数据位置目标与当前目录相同、原生运行时位置和配置路径均未变化且没有未完成恢复时，`configure_storage` 只提交布局，不得把已有 bootstrap 误判为 OpenClaw 已安装，也不得要求缺失的 OpenClaw 二进制核验 Gateway 服务。真实迁移、会改变服务启动绑定的变更和未完成恢复仍必须先核验服务身份。
- 未完成运行时恢复只有在持久事务证明先前存在所选官方 Gateway 服务时，才需要解析服务专用 OpenClaw 与 Node.js 启动契约。事务明确记录没有所选服务时，回滚必须恢复布局并继续核验端口和先前运行状态，不能被无关的 Node.js 兼容性检查阻断。
- 运行时恢复失败页必须区分“正在恢复”与“恢复失败”。失败时主文案说明可执行的依赖修复路径，原始诊断默认收起；“上一步”只退出当前页面并保留待恢复事务，不能再次隐式执行同一失败回滚。用户回到数据位置页后仍必须完成恢复才能继续修改位置。
- 候选 Gateway 端口被占用时，失败页必须先识别并展示脱敏后的占用进程。只有同一恢复端口、同一 PID、同一启动代次且属于当前用户的进程可进入确认操作；确认后先请求正常退出，超时才强制结束，端口释放后自动继续原恢复事务。占用者变化、跨用户或无法核验时不得终止。
...
- Gateway 就绪终态必须原子收敛执行步骤：Native 路径中已证明可运行的 Node.js、OpenClaw 和 Gateway 为完成，本次未参与安装且没有既有完成事实的 npm 为已跳过；Docker 路径中的容器和 Gateway 为完成。成功摘要不得与进行中动画、等待状态或非 100% 的步骤进度并存。
...
- 首次安装即使已经生成 bootstrap，只要选择当前数据目录且没有服务绑定变更，也能在 OpenClaw 尚未安装时提交并进入运行时安装；相同条件不得触发官方 Gateway 服务核验错误。
- 没有所选官方 Gateway 服务的待恢复事务不要求兼容 Node.js 即可回滚布局；存在已登记服务或先前运行中的 Native Gateway 时仍保留对应运行时和健康核验门禁。
- 已安装 OpenClaw 因 Node.js 不兼容而不能执行时，数据位置事务必须先通过平台服务痕迹、JunQi 自有运行时和端口只读证据确认不存在待保护服务，再进入运行时步骤自动修复 Node.js。真实数据迁移还必须核验 Docker 残留；当前位置仅调整 Native 依赖时不得因 Docker 守护进程不可用而阻断。当前操作范围内任一证据存在或不可核验时不得修改存储或服务。
- 平台痕迹证明已登记 Gateway 服务、但当前 Node.js 无法执行 OpenClaw 时，数据位置页必须显示本地化的 Node 修复主操作。用户显式点击后复用可取消的 Node 安装和 npm 修复契约，只有 Node 与 npm 均核验成功才自动重试原 `configure_storage`；任一失败继续保留服务归属门禁，不伪造成功。
- macOS、Linux 和 Windows 的系统 Node.js 发现必须核验 PATH 内全部可执行候选。排在首位的不兼容版本只作为失败诊断保留；后续满足目标 `engines.node` 且提供同发行版 npm 的候选必须被选中，安装完成后不得因旧版本仍在 PATH 前部而要求用户重复下载安装。
- 恢复失败页的“上一步”能够返回先前用户页面，且不会清除、提交或重复回滚待恢复事务；重试按钮保持单飞，失败后恢复可操作状态。
- 端口占用恢复页在识别期间禁用主操作；识别成功后显示进程名、PID 与 OpenClaw 相关性提示，使用共享确认对话框说明任务中断风险。确认执行前后都重新核验端口所有者，恢复成功前不得清除持久事务或显示成功。
- 官方服务停止前必须从同一已核验 OpenClaw 运行时读取 `gateway stop --help`。只有帮助契约声明 `--force` 时才携带该参数；未声明时只执行裸停止。帮助失败不得执行停止，停止失败后不得换参数重放未知副作用。
...
- 已安装 OpenClaw 的自动更新检查必须在 React 开发模式的 Effect 清理与重建后继续接受同一次 Tauri 请求的结构化终态；进度达到 100% 不得被客户端推断为成功，只有结构化返回可以结束检查并按更新策略解锁下一步。真实卸载和后续请求换代仍必须拒绝旧结果。
- `openclaw.setup.detect` 必须按当前官方 schema 接受 `saved-auth:*` 候选、`install` 与 `custom` 授权类型、可选的不可用项和推荐安装列表，以及可选原生会话目录字段；字段存在但类型、非空约束或 HTTPS 约束不成立时仍必须失败关闭。
