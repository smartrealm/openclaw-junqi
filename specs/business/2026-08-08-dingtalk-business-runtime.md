...
22. 能力表格是业务工作台的主要浏览区。搜索、业务域和操作效果等高频本地筛选必须在可展开筛选栏中集中呈现；筛选栏不得与工具详情重复承载租户身份。租户身份仅作为当前工具调用的显式参数保留在工具详情中，不得持久化。
...
28. 业务侧栏必须利用现有窗口空间展示当前平台、当前 Session、当前 Agent、有效工具摘要与最近本窗口操作；不得以静态数量或伪造运行状态填充空白区域。
29. 工具筛选栏只负责搜索、业务域和操作效果，Profile 仅在工具详情与调用参数边界出现，不能在同一工作区重复维护两份输入状态。
30. 操作审计摘要只能从 OpenClaw 官方 metadata-only 事件和本窗口调用投影派生；参与 Agent、处理中和需关注计数不得推断委派关系或业务终态。
31. 插件安装和 Gateway 重启必须展示不伪造百分比的不定进度；异步失败必须保留确认上下文并内联显示错误，不能关闭弹层形成完成错觉。
32. `restartRequired` 与插件安装条件同时成立时必须优先引导重启。当前 Gateway 未提供已验证桌面变更边界时，不得允许安装操作，只能说明边界并重新检测。
