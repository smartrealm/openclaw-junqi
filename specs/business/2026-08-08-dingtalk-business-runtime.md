...
19. DWS 缺失引导必须明确安装目标绑定当前 Gateway runtime，并仅展示已核对的官方安装/登录入口；复制命令、打开官方文档和重新检测均为用户主动操作，JunQi 不执行远程脚本或读取 token。
20. 钉钉业务插件未就绪时必须在工作台显示“在 JunQi 安装”入口；安装动作仅允许对已验证的当前 Gateway 执行，安装成功不等于工具已进入当前 Session，必须重启 Gateway 并重新读取工具状态。
21. 业务应用侧栏必须提供“有效工具”“操作审计”“接入与授权”三个稳定入口；页面内部不得再复制一套工具与记录页签。
22. 工具与审计视图在 readiness 完全就绪时不得持续占用状态条空间；任何阻断、错误或待核验状态仍需内联显示并提供真实动作。
23. “接入与授权”必须分别展示 Session、插件、Agent 双层授权和 DWS 身份核验，不得合并成单一“已连接”状态。
24. 工具表格只能按当前 `tools.effective` 返回的真实工具业务域分组，不得生成静态能力数量、占位工具或推测性可用状态。
25. 工具详情优先展示业务域、效果、风险和 Session 状态；工具 ID、DWS canonical path、schema 摘要和 JSON 参数进入可访问的高级披露区。
26. 业务侧栏必须利用现有窗口空间展示当前平台、当前 Session、当前 Agent、有效工具摘要与最近本窗口操作；不得以静态数量或伪造运行状态填充空白区域。
27. 工具筛选栏只负责搜索、业务域和操作效果，Profile 仅在工具详情与调用参数边界出现，不能在同一工作区重复维护两份输入状态。
28. 操作审计摘要只能从 OpenClaw 官方 metadata-only 事件和本窗口调用投影派生；参与 Agent、处理中和需关注计数不得推断委派关系或业务终态。
29. 插件安装和 Gateway 重启必须展示不伪造百分比的不定进度；异步失败必须保留确认上下文并内联显示错误，不能关闭弹层形成完成错觉。
30. `restartRequired` 与插件安装条件同时成立时必须优先引导重启。当前 Gateway 未提供已验证桌面变更边界时，不得允许安装操作，只能说明边界并重新检测。
...
- 前端：补充插件更新后的重启优先级、确认操作不定进度、确认失败保留、审计摘要派生与来源筛选回归。
